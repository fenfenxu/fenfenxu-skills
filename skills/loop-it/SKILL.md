...
5. **建/修唯一巡检 Autopilot**（在已绑定 workspace 下）：title `Loop Patrol`；agent=`orchestrator`；mode=`run_only`。description **必须内嵌完整巡检程序**（由 config 填空；`repo` 为 `owner/name` 时 checkout URL 用 `https://github.com/<repo>.git`；已是 URL 则原样）。改 `base_branch` / `repo` / 巡检规则后必须再 `autopilot update` 同步 description：
   ...
   ```text
   你是 Loop Patrol（run_only）。本 description 即程序（已注入；勿再 get 自己）。无动作则静默。裸 multica，勿加 --profile。

   ## 启动
   1. multica repo checkout <repo_clone_url> --ref <base_branch>
   2. 读 .loop-it/config.yaml，核对 repo / base_branch。

   ## 巡检（按序；结论未变则尽早停）
   只改 Multica issue/评论，或合规时 gh pr merge --repo <repo>。不改代码、plan、.loop-it；勿 push 基线。

   1. 日报：若 daily_digest 开启，只看 daily_digest.issue_key 最近几条 root 摘要；今日已有 Patrol 汇总则跳过，否则写一条。
   2. 找 loop_it_phase=executing 的主任务；用 issue get + 近评摘要（或本 autopilot 最近 runs）判断上轮结论是否仍成立。
   3. 结论仍成立且主任务活动时间未更新 → 直接收尾（不读 children、不看 PR、不 list autopilot、不 sweep、不评论）。
   4. 否则：读 children/近评，解 blocked 或验收；auto_merge 且 DoD+CI 绿才 merge；有实质动作才留评。再按需 sweep 未关闭的 Loop It/Loop Patrol 运行 issue（最多一页）。
   5. 子任务全完 → 主任务 done（phase=completed）；不停本 Autopilot。

   ## 收尾
   run_only 无关联 issue 则结束。
   ```
...
1. **日报**：若开启，只看近几条 root 摘要；今日已有则跳过，否则写一条。
2. **轻量探测** executing 主任务（get + 近评 / 最近 runs）。结论未变且活动未更新 → **直接收尾**（不读 children / PR / sweep）。
3. 否则读 children + 近评；解 blocked 或验收；`auto_merge` 且 DoD+CI 过则 merge；有动作才留评。再按需一页 sweep。
4. 子任务全完 → Phase 5。per-task autopilot 默认不 list。
